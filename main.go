...
func timedCopy(dst io.Writer, src io.Reader, timeout time.Duration) (written int64, err error) {
	...
	for {
		...
		if nr > 0 {
			...
			if nw < 0 || nr < nw { nw = 0; if ew == nil { ew = io.ErrShortWrite } }; written += int64(nw)
...
func handleDirectTCPIP(ch ssh.Channel, destHost string, destPort uint32, remoteAddr net.Addr) {
	...
	if destPort == 7300 {
		log.Printf("Detected UdpGw request on port 7300 from %s", remoteAddr)
		handleUdpGw(ch, remoteAddr)
		return
	}
	...
	go func() {
		defer func() {
			if tcpConn, ok := destConn.(*net.TCPConn); ok { tcpConn.CloseWrite() }
			close(done)
		}()
...
func httpHandshake(conn net.Conn) (net.Conn, error) {
	...
	for {
		...
		if strings.Contains(req.UserAgent(), expectedUA) {
			...
			log.Printf("Incorrect handshake payload from %s (UA: %s).", conn.RemoteAddr(), req.UserAgent())
...
func handleSshConnection(c net.Conn, sshCfg *ssh.ServerConfig) {
	defer func() {
		if r := recover(); r != nil { log.Printf("FATAL: Panic recovered for %s: %v", c.RemoteAddr(), r) }
		c.Close()
	}()
...
func main() {
	...
	log.Println("====== WSTUNNEL (TCP Proxy + UdpGw Mode) Starting ======")
	...
	log.Printf("SSH server listening on %s. UdpGw traffic will be handled on port 7300.", globalConfig.ListenAddr)
	for {
