...
func sendKeepAlives(sshConn ssh.Conn, done <-chan struct{}) {
	...
	for {
		...
		case <-ticker.C:
			...
			if err != nil {
				// Don't log here to avoid spam on normal disconnects
				return
			}
		case <-done:
			return
...
func handleSshConnection(c net.Conn, sshCfg *ssh.ServerConfig) {
	...
	finalReader := io.MultiReader(bytes.NewReader(preReadData), c) // 使用 c 而不是 reader
...
func main() {
	...
	if globalConfig.BufferSizeKB <= 0 { globalConfig.BufferSizeKB = 8 }
