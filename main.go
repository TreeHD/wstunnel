...
func tolerantCopy(dst io.Writer, src io.Reader, direction string, remoteAddr net.Addr, username string) {
	...
	for {
		...
		if nr > 0 {
			...
			if nw > 0 {
				if direction == "Client->Target" { atomic.AddUint64(&traffic.Sent, uint64(nw))
				} else { atomic.AddUint64(&traffic.Received, uint64(nw)) }
			}
...
func apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	case r.URL.Path == "/api/connections":
		...
		onlineUsers.Range(func(_, v interface{}) bool {
			u := v.(*OnlineUser)
			globalConfig.lock.RLock(); acc, ok := globalConfig.Accounts[u.Username]; globalConfig.lock.RUnlock()
			if !ok { return true }
			t_val, _ := globalTraffic.LoadOrStore(u.Username, &TrafficInfo{}); t := t_val.(*TrafficInfo)
	...
	case r.URL.Path == "/api/accounts/set_status" && r.Method == "POST":
		...
		if acc, ok := globalConfig.Accounts[payload.Username]; ok {
			acc.Enabled = payload.Enabled; globalConfig.Accounts[payload.Username] = acc
			if !payload.Enabled { onlineUsers.Range(func(_, v interface{}) bool { u := v.(*OnlineUser); if u.Username == payload.Username { u.sshConn.Close() }; return true }) }
		}
	...
	case r.URL.Path == "/api/admin/update_password" && r.Method == "POST":
		...
		if globalConfig.AdminAccounts[user] == payload.OldPassword {
			globalConfig.AdminAccounts[user] = payload.NewPassword; safeSaveConfig(); sendJSON(w, http.StatusOK, map[string]string{"message": "密码更新成功"})
	...
	case r.URL.Path == "/api/accounts/reset-traffic":
		var p struct { Username string `json:"username"`}; json.NewDecoder(r.Body).Decode(&p)
...
func main() {
	...
	go func() {
		...
		mux.HandleFunc("/", authMiddleware(func(w http.ResponseWriter, r *http.Request) { if r.URL.Path == "/" { http.ServeFile(w, r, "admin.html"); return }; http.NotFound(w,r) }))
	...
	sshCfg := &ssh.ServerConfig{
		...
		PasswordCallback: func(c ssh.ConnMetadata, p []byte) (*ssh.Permissions, error) {
			user := c.User()
			globalConfig.lock.RLock(); acc, ok := globalConfig.Accounts[user]; globalConfig.lock.RUnlock()
